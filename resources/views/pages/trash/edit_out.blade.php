...
<div class="container">
    <div class="col-md-12 d-flex justify-content-center flex-column align-items-center">
        <h1>Editar cadastro de saída</h1>
        <div class="col-md-8 mb-5">
            <form method="POST" action="{{route('trash.save')}}">
                ...
                <div class="form-group">
                    <label for="destino">Destino do Lixo:</label>
                    <select class="form-control" name="destino" id="destino">
                        @foreach ($destinations as $destination)
                            <option {{$destination->id == $trash->destination->id ? 'selected' : ''}} value="{{$destination->id}}">{{$destination->name}}</option>
                        @endforeach
                      </select>
                </div>
